...
@dp.callback_query_handler(lambda query: 'category_data' in query.data)
async def handle_category_callback(callback: CallbackQuery):
    ...
    for product in await db.get_products_by_category(category):
        # Retrieve message and reply markup for the product card
        msg, keyboard = await get_product_card_info_msg(
...
@dp.callback_query_handler(lambda query: 'back_to_catalogue_data' in query.data)
...
@dp.callback_query_handler(lambda query: 'add_to_cart_data' in query.data)
...
async def handle_remove_from_cart_callback(callback: CallbackQuery):
    ...
    product, user = await db.get_product_by_id(product_id), await db.get_user_by_id(callback.from_user.id)
...
async def successful_payment(message: Message):
    ...
    await bot.send_message(message.chat.id, await successful_payment_msg(
